 ...
 * must contain a single `permalinks-for-hivepress` directory.
 *
 * @package Permalinks
 */

namespace Permalinks\Updater;

// Exit if accessed directly.
defined( 'ABSPATH' ) || exit;

const UPDATE_REPO = 'irapidchris-del/permalinks-for-hivepress';

const UPDATE_SLUG = 'permalinks-for-hivepress';

const UPDATE_CACHE_KEY = 'permalinks_for_hivepress_release';

/**
 * Why the last release check came back empty, so the notice can say which.
 */
const UPDATE_REASON_KEY = 'permalinks_for_hivepress_release_reason';
 ...
 */
const UPDATE_RATE_LIMIT_KEY = 'permalinks_for_hivepress_release_rate_limit';
...
function get_plugin_information( $result, $action, $args ) {
	...
	return (object) [
		...
		'donate_link'   => function_exists( 'hppl_get_support_url' ) ? hppl_get_support_url() : '',
		'sections'      => [
			'description' => wpautop( esc_html( $plugin_data['Description'] ) ),
			'changelog'   => $release['notes'] ? wpautop( esc_html( $release['notes'] ) ) : '<p>' . esc_html__( 'See the GitHub releases page for the changelog.', 'permalinks-for-hivepress' ) . '</p>',
		],
	];
}
...
function add_update_check_link( $links ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$links[] = '<a href="' . esc_url( wp_nonce_url( self_admin_url( 'plugins.php?permalinks_check_updates=1' ), 'permalinks_check_updates' ) ) . '">' . esc_html__( 'Check for updates', 'permalinks-for-hivepress' ) . '</a>';
	}
...
function handle_update_check() {
	if ( ! isset( $_GET['permalinks_check_updates'] ) || ! current_user_can( 'update_plugins' ) ) {
		return;
	}

	check_admin_referer( 'permalinks_check_updates' );
	...
	wp_safe_redirect( add_query_arg( 'permalinks_checked', $status, self_admin_url( 'plugins.php' ) ) );
...
function show_update_check_notice() {
	...
	if ( ! isset( $_GET['permalinks_checked'] ) || ! current_user_can( 'update_plugins' ) ) {
		return;
	}

	$status = sanitize_key( wp_unslash( $_GET['permalinks_checked'] ) );
	...
	if ( 'available' === $status ) {
		...
		$message = sprintf( __( 'A new version of Permalinks for HivePress (%s) is available.', 'permalinks-for-hivepress' ), $release ? $release['version'] : '' );
		$class   = 'notice-success';
	} elseif ( 'none' === $status ) {
		$message = __( 'Permalinks for HivePress is up to date.', 'permalinks-for-hivepress' );
		$class   = 'notice-success';
	} elseif ( 'norelease' === $status ) {
		$message = __( 'Permalinks for HivePress has no releases published yet, so there is nothing to update to. This is normal for a new install and nothing is wrong.', 'permalinks-for-hivepress' );
		$class   = 'notice-info';
	} elseif ( 'empty' === $status ) {
		$message = __( 'No releases have been published for Permalinks for HivePress yet, so there is nothing to update to. This is normal for a brand new copy and does not mean anything is wrong.', 'permalinks-for-hivepress' );
		$class   = 'notice-info';
	} elseif ( 'limited' === $status ) {
		$message = __( 'GitHub limits how many update checks one server may make each hour, and this server has reached that limit. Nothing is wrong with the plugin or your site, and checking will work again within the hour.', 'permalinks-for-hivepress' );
		$class   = 'notice-warning';
	} elseif ( 'error' === $status ) {
		$message = __( 'Could not reach GitHub to check for updates. Please try again later.', 'permalinks-for-hivepress' );
		$class   = 'notice-error';
	} else {
		return;
	}
...
function fix_update_directory( $source, $remote_source, $upgrader, $hook_extra = [] ) {
	...
	if ( ! $wp_filesystem->move( untrailingslashit( $source ), untrailingslashit( $target ) ) ) {
		return new \WP_Error( 'permalinks_rename_failed', __( 'Could not rename the update directory.', 'permalinks-for-hivepress' ) );
	}
...
function add_bulk_check( $actions ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$actions['hpx_check_updates'] = __( 'Check for updates', 'permalinks-for-hivepress' );
	}
...
function show_bulk_check_notice() {
	...
	if ( $available ) {
		...
		$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'permalinks-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
	} else {
		/* translators: %s: number of plugins checked. */
		$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'permalinks-for-hivepress' ), number_format_i18n( $checked ) );
	}
...
if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
	$GLOBALS['hpx_update_check_bulk'] = 'permalinks-for-hivepress'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
